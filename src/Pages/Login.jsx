...
export default function Login({ onLogin, isAuthenticated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { token, role, _id, email: userEmail, name, firstLogin } = response.data.data;

      // Store session details (both sessionStorage and localStorage for persistence)
      ...
      // First login flow – show password change modal
      if (firstLogin) {
        setUserId(_id);
        setShowModal(true);
        return; // Modal will handle password change then redirect
      }
